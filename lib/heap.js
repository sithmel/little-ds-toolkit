...
Heap.bubbleUp = function (data, i, comp) {
  ...
  while (i > 0) {
    ...
    if (comp(data[i], data[parentIndex])) {
...
Heap.bubbleDown = function (data, i, comp) {
  ...
  while (i < data.length) {
    ...
    child1isSmaller = typeof data[child1Index] !== 'undefined' ? comp(data[child1Index], data[i]) : false;
    child2isSmaller = typeof data[child2Index] !== 'undefined' ? comp(data[child2Index], data[i]) : false;
    if (child1isSmaller && child2isSmaller) {
      if (comp(data[child1Index], data[child2Index])) {
